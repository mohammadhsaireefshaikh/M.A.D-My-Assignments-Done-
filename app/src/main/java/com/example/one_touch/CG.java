...
public class CG extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("CG");
        getSupportActionBar().setDefaultDisplayHomeAsUpEnabled(true);

        download2=findViewById(R.id.link);
        download2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                storageReference=firebaseStorage.getInstance().getReference();
                ref=storageReference.child("comp/SE/CG/computergraphicslabmanual-161126200024.pdf");

                ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        String url=uri.toString();
                        downloadFile(CG.this,"CG_writeups",".pdf",DOWNLOAD_SERVICE,url);
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(CG.this, "Downloading Failed ", LENGTH_SHORT).show();
    ...
    public void view2(View view) {
        ...
        Intent intent = new Intent(CG.this, CGsyllabi.class);
